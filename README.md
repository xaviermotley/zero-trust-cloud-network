...
```mermaid
graph LR
    User[End Users / Devices] -->|Authenticate| IdP[Identity Provider]
    IdP -->|Issue tokens| AWSVerifiedAccess[Verified Access]
    AWSVerifiedAccess -->|Access Policies| App[VPC Lattice Services]
    App -->|Routes| Resources[Applications / APIs / Services]
    subgraph Traditional Network Controls
        Firewall[Firewall / Security Groups]
        VPN[VPN / Direct Connect]
    end
    User --> Firewall --> App
    User --> VPN --> App
    App -->|Logs| Monitoring[CloudWatch / GuardDuty]
```
</details>
...
This repository uses separate branches to stage different parts of the Zero Trust journey:

- **main** – Stable reference implementation and documentation.
- **lab/** – Experiments and lab exercises based on the AWS re:Inforce session.
- **templates/** – CloudFormation / Terraform templates for deploying example architectures.
